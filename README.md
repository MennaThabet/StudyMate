...
1. (Optional) Upload course documents in the sidebar and click **Index documents**
   — this routes them into the Researcher's knowledge source.
...
6. Once approved, the final report renders inline and can be exported as
   Markdown via the download button.
...
*(To be added after the first full run: objective entered → reasoning stream →
failed quality check → retry → approval → exported Markdown.)*
